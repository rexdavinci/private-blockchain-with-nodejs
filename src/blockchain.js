...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            // check for the height to assign prevBlockHash
            ...
            // increment chain and block heights
            const nextHeight = self.height + 1;
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(!bitcoinMessage.verify(message, address, signature)) {
                reject("Invalid Message or signature")
            }
    ...
    validateChain() {
        let self = this;
        let errorLog = [];
        const auxiliaryChain = self.auxiliaryChain // to include block not added yet
        return new Promise(async (resolve, reject) => {
            for(let blockIndex in auxiliaryChain) {
                const block = auxiliaryChain[blockIndex];
                const isValid = await block.validate()

                // error type - invalid block
                if(!isValid) {
                    errorLog.push(`Block ${blockIndex} is invalid`)
                }
                
                // Each Block should check the with the previousBlockHash to make sure the chain isn't broken.
                let isBroken = false;
                if(blockIndex > 0) {
                    isBroken = auxiliaryChain[blockIndex].previousBlockHash !== auxiliaryChain[blockIndex - 1].hash
                }

                // error type - chain is broken
                if(isBroken) {
